...
class ChallengeSixXSS(Exploit):
    ...
    def run_command(self, command):
        raise UnsupportedException("Running commands with ChallengeSixXSS is not supported.")

    def run_hellevator(self):
        raise UnsupportedException("Running Hellevator with ChallengeSixXSS is not supported.")
    ...
    def test_if_vulnerable(self):
        try:
            ...
            logger.info(f"{BetterLogger.COLOR_PINK}{self.ip_address} - Unable to test ChallengeSixXSS because the student disabled the apache2 service!{BetterLogger.COLOR_END}")
            return None
        except Exception as err:
            logger.info(f"{BetterLogger.COLOR_RED}{self.ip_address} - Something went wrong while checking if ChallengeSixXSS is vulnerable.{BetterLogger.COLOR_END}")
